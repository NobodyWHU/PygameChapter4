...
def main():
    ...
    while True:
        ...
        checkForQuit()
        for event in pygame.event.get():  #evnet handling loop
            if event.type==MOUSEBUTTONUP:
                ...
                if(spotx, spoty)==(None, None):
                    ...
                    if RESET_RECT.collidepoint(event.pos):
                        resetAnimation(mainBoard, allMoves) #click on the Reset Button
                        allMoves=[]
                    elif NEW_RECT.collidepoint(event.pos):
                        mainBoard, solutionSeq=generateNewPuzzle(80) #click on the New Game Butoon
                        allMoves=[]
                    elif SOLVE_RECT.collidepoint(event.pos):
                        resetAnimation(mainBoard, solutionSeq+allMoves) #click on the solve Button
                        allMoves=[]
                else:
...
def getStartingBoard():
    ...
    #For example,if BOARDWIDTH and BOARDHEIGHT are both 3,this funtion return
...
def drawTile(tilex, tiley, number, adjx=0, adjy=0):
    ...
    #pixel over (determined by adjx and adjy)
    left, top=getLeftTopOfTile(tilex,tiley)
